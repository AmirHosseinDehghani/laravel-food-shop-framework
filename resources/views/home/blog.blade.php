    ...
    <div class="container-fluid">
        <div class="row">
            <div class="section-header d-flex align-items-center justify-content-between my-5">
                <h2 class="section-title">بلاگ های جدید فروشگاه</h2>
            </div>
        </div>
        <div class="row">
            @foreach($blogs as $blog)
                <div class="col-md-4 d-flex">
                    <article class="post-item card border-0 shadow-sm p-3 w-100 d-flex flex-column">
                        <div class="image-holder zoom-effect">
                            <img style="width: 200px ; height: 200px" src="{{asset("storage/$blog->img")}}" alt="post" class="card-img-top">
                        </div>
                        <div class="card-body flex-grow-1 d-flex flex-column">
                            <div class="post-header mb-auto">
                                @foreach($users as $user)
                                    @if($user->id == $blog->writer)
                                        <h4 class="post-title">
                                            <a href="#" class="text-decoration-none">{{$user->name . ' ' . $user->family}}</a>
                                        </h4>
                                    @endif
                                @endforeach
                                <h4 class="post-title">
                                    <a href="#" class="text-decoration-none">{{$blog->subject}}</a>
                                </h4>
                                <p>{!! $blog->blog !!}</p>
                            </div>
                        </div>
